...
export function SiteHeader() {
  ...
  return <><div className="topline"><div className="container-shell flex items-center justify-between py-2 text-xs font-semibold text-white/75"><span>فرص موثوقة من شركات الخليج</span><span className="hidden sm:block">العربية | English</span></div></div>
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl"><div className="container-shell flex h-[78px] items-center justify-between gap-6">
      ...
      <nav className="hidden items-center gap-7 text-sm font-black text-slate-600 lg:flex"><a href="/">الرئيسية</a><a href="/jobs">الوظائف</a><a href="/#countries">الدول</a><a href="/#specialties">التخصصات</a><a href="/companies">الشركات</a><a href="/salaries">الرواتب</a><a href="/articles">دليل التوظيف</a></nav>
    ...
    </div>{open && <nav className="container-shell grid gap-1 border-t py-4 text-sm font-black lg:hidden">{[["الرئيسية","/"],["الوظائف","/jobs"],["الدول","/#countries"],["التخصصات","/#specialties"],["الشركات","/companies"],["الرواتب","/salaries"],["دليل التوظيف","/articles"],["English","/en"]].map(([label,href])=><a key={label} onClick={()=>setOpen(false)} className="rounded-lg px-3 py-3 hover:bg-emerald-50" href={href}>{label}</a>)}</nav>}</header></>;
}
