...
async function findJob(slug:string):Promise<Job|undefined>{try{const [row]=await getDb().select().from(jobsTable).where(eq(jobsTable.slug,slug)).limit(1);if(row)return{slug:row.slug,title:row.titleAr,company:row.company,city:row.city,country:row.country,type:row.employmentType,mode:row.workMode,age:"حديثاً",badge:row.featured?"مميزة":"جديدة",initials:row.company.split(/\s+/).map(x=>x[0]).join("").slice(0,2).toUpperCase(),category:row.category,salary:row.salary,description:row.descriptionAr,requirements:JSON.parse(row.requirementsAr||"[]") as string[]};}catch{}return demoJobs.find(j=>j.slug===slug);}
export async function generateMetadata({params}:{params:Promise<{slug:string}>}) { const {slug}=await params; const job=await findJob(slug); return {title:job?`${job.title} في ${job.city} | توطين`:"الوظيفة غير موجودة",description:job?.description}; }
