...
export function JobsClient({initialJobs,initialQuery="",initialCountry="all"}:{initialJobs:Job[];initialQuery?:string;initialCountry?:string}) {
  ...
  const key=(value:string)=>{const text=value.trim().toLowerCase().replace(/[أإآ]/g,"ا").replace(/ُ/g,"");if(text.includes("سعود")||text==="sa"||text.includes("saudi"))return"SA";if(text.includes("امارات")||text==="ae"||text==="uae"||text.includes("united arab emirates"))return"AE";if(text.includes("قطر")||text==="qa"||text.includes("qatar"))return"QA";if(text.includes("كويت")||text==="kw"||text.includes("kuwait"))return"KW";if(text.includes("عمان")||text==="om"||text.includes("oman"))return"OM";if(text.includes("بحرين")||text==="bh"||text.includes("bahrain"))return"BH";return text};
  const filtered=useMemo(()=>jobs.filter(j=>(!query||`${j.title} ${j.company} ${j.city} ${j.category}`.includes(query))&&(country==="all"||key(j.country)===key(country))&&(category==="all"||j.category===category)),[jobs,query,country,category]);
